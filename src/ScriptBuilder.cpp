...
static string RemoveWhitespace(const string &value)
{
	...
	{
		if( isspace(static_cast<unsigned char>(value[i])) == 0 )
			result += value[i];
	}
	return result;
}
...
static bool StartsArrayTemplate(const string &value, size_t pos, size_t &openPos)
{
	...
	static const size_t keywordLength = 5;
	if( value.compare(pos, keywordLength, keyword) != 0 )
		return false;
	if( pos > 0 && IsIdentifierChar(value[pos - 1]) )
		return false;

	size_t cursor = pos + keywordLength;
	if( cursor < value.size() && IsIdentifierChar(value[cursor]) )
		return false;
	while( cursor < value.size() && isspace(static_cast<unsigned char>(value[cursor])) != 0 )
		cursor++;
...
static size_t FindMatchingAngleBracket(const string &value, size_t openPos)
{
	int depth = 0;
...
static string NormalizeArrayTemplateSyntax(const string &value)
{
	string result;
	result.reserve(value.size());
	for( size_t i = 0; i < value.size(); )
	{
		...
		if( StartsArrayTemplate(value, i, openPos) )
		{
			size_t closePos = FindMatchingAngleBracket(value, openPos);
			if( closePos != string::npos )
			{
				result += NormalizeArrayTemplateSyntax(value.substr(openPos + 1, closePos - openPos - 1));
				result += "[]";
				i = closePos + 1;
				continue;
			}
...
static string StripParameterName(const string &parameter)
{
	string result = TrimString(parameter);
	if( result.empty() )
		return result;

	size_t begin = result.size();
	while( begin > 0 && IsIdentifierChar(result[begin - 1]) )
		begin--;
	...
	if( !hasSeparator || beforeName == 0 )
		return result;

	return TrimString(result.substr(0, beforeName));
}

static string StripParameterDefaultValue(const string &parameter)
{
	int angleDepth = 0;
	int parenDepth = 0;
	int bracketDepth = 0;
...
static vector<string> SplitParameterList(const string &parameters)
{
	...
	result.push_back(parameters.substr(begin));
	return result;
}

static string NormalizeDeclarationForMetadataLookup(const string &declaration)
{
	size_t open = declaration.find('(');
	if( open == string::npos )
		return RemoveWhitespace(NormalizeArrayTemplateSyntax(declaration));
	...
	string result = RemoveWhitespace(NormalizeArrayTemplateSyntax(declaration.substr(0, open)));
	result += "(";

	vector<string> params = SplitParameterList(declaration.substr(open + 1, close - open - 1));
	bool wroteParam = false;
	for( size_t i = 0; i < params.size(); ++i )
	{
		string param = RemoveWhitespace(NormalizeArrayTemplateSyntax(StripParameterName(StripParameterDefaultValue(params[i]))));
		if( param.empty() )
			continue;
		if( wroteParam )
			result += ",";
	...
	result += ")";
	result += RemoveWhitespace(NormalizeArrayTemplateSyntax(declaration.substr(close + 1)));
	return result;
}

static bool MetadataDeclarationMatches(asIScriptFunction *function, const string &requestedDeclaration)
{
	if( function == 0 || requestedDeclaration.empty() )
		return false;

	const char *declarationWithNamespaces = function->GetDeclaration(false, true, false);
	if( declarationWithNamespaces != 0 &&
		NormalizeDeclarationForMetadataLookup(declarationWithNamespaces) == requestedDeclaration )
	{
		return true;
	}

	const char *declarationWithoutNamespaces = function->GetDeclaration(false, false, false);
	return declarationWithoutNamespaces != 0 &&
		NormalizeDeclarationForMetadataLookup(declarationWithoutNamespaces) == requestedDeclaration;
}

static asIScriptFunction *FindFunctionByMetadataDeclaration(asIScriptModule *module, const string &nameSpace, const string &name, const string &declaration)
{
	if( module == 0 )
		return 0;

	string requestedDeclaration = NormalizeDeclarationForMetadataLookup(declaration);
	if( requestedDeclaration.empty() )
		return 0;
	...
	for( asUINT i = 0; i < module->GetFunctionCount(); ++i )
	{
		...
		const char *functionNamespace = function->GetNamespace();
		if( nameSpace != (functionNamespace ? functionNamespace : "") )
			continue;

		const char *functionName = function->GetName();
		if( !name.empty() && (functionName == 0 || name != functionName) )
			continue;

		if( MetadataDeclarationMatches(function, requestedDeclaration) )
			return function;
	}
...
static asIScriptFunction *FindMethodByMetadataDeclaration(asITypeInfo *type, const string &name, const string &declaration)
{
	if( type == 0 )
		return 0;
	...
	{
		asIScriptFunction *method = type->GetMethodByIndex(i, false);
		if( method == 0 )
			continue;

		const char *methodName = method->GetName();
		if( !name.empty() && (methodName == 0 || name != methodName) )
			continue;

		if( MetadataDeclarationMatches(method, requestedDeclaration) )
			return method;
	}
...
void ScriptBuilder::SetIncludeCallback(ScriptBuilderIncludeCallback callback, void *userParam)
{
	includeCallback = callback;
	includeParam   = userParam;
}

void ScriptBuilder::SetPragmaCallback(ScriptBuilderPragmaCallback callback, void *userParam)
{
	pragmaCallback = callback;
	pragmaParam = userParam;
}

int ScriptBuilder::StartNewModule(asIScriptEngine *inEngine, const char *moduleName)
{
...
asIScriptEngine *ScriptBuilder::GetEngine()
{
	return engine;
}

asIScriptModule *ScriptBuilder::GetModule()
{
	return module;
}

unsigned int ScriptBuilder::GetSectionCount() const
{
	return (unsigned int)(includedScripts.size());
}

string ScriptBuilder::GetSectionName(unsigned int idx) const
{
...
int ScriptBuilder::AddSectionFromFile(const char *filename)
{
...
// Returns 0 if the section was not included because it had already been included before
// Returns <0 if there was an error
int ScriptBuilder::AddSectionFromMemory(const char *sectionName, const char *scriptCode, unsigned int scriptLength, int lineOffset)
{
...
int ScriptBuilder::BuildModule()
{
	return Build();
}

void ScriptBuilder::DefineWord(const char *word)
{
	string sword = word;
...
bool ScriptBuilder::IncludeIfNotAlreadyIncluded(const char *filename)
{
...
int ScriptBuilder::LoadScriptSection(const char* filename)
{
...
int ScriptBuilder::ProcessScriptSection(const char *script, unsigned int length, const char *sectionname, int lineOffset)
{
...
int ScriptBuilder::Build()
{
...
#if AS_PROCESS_METADATA == 1
	struct DefaultNamespaceRestore
	{
		asIScriptModule *module;
		string nameSpace;
		~DefaultNamespaceRestore()
		{
			if( module )
				module->SetDefaultNamespace(nameSpace.c_str());
		}
	};
	const char *previousNamespace = module->GetDefaultNamespace();
	DefaultNamespaceRestore namespaceRestore = { module, previousNamespace ? previousNamespace : "" };
	auto writeMetadataWarning = [this](const SMetadataDecl &decl, const char *reason)
	{
		string message = "ScriptBuilder metadata was not bound";
		if( reason && reason[0] )
		{
			message += ": ";
			message += reason;
		...
		message += " (declaration='";
		message += decl.declaration;
		message += "'";
		if( !decl.parentClass.empty() )
		{
			message += ", parent='";
			message += decl.parentClass;
			message += "'";
		}
		if( !decl.nameSpace.empty() )
		{
			message += ", namespace='";
			message += decl.nameSpace;
			message += "'";
		}
		message += ")";
		engine->WriteMessage(module && module->GetName() ? module->GetName() : "", 0, 0, asMSGTYPE_WARNING, message.c_str());
	};
	...
	for( int n = 0; n < (int)foundDeclarations.size(); n++ )
	{
		...
		if( module->SetDefaultNamespace(decl->nameSpace.c_str()) < 0 )
		{
			writeMetadataWarning(*decl, "failed to set namespace");
			continue;
		}
		...
		else if( decl->type == MDT_FUNC )
		{
			if( decl->parentClass == "" )
			{
				...
				if( func == 0 )
					func = FindFunctionByMetadataDeclaration(module, decl->nameSpace, decl->name, decl->declaration);
				assert( func );
				if( func )
				{
			...
			else
			{
				...
				asIScriptFunction *func = FindMethodByMetadataDeclaration(type, decl->name, decl->declaration);
				assert( func );
				if( func )
				{
		...
		else if( decl->type == MDT_VIRTPROP )
		{
			...
			else
			{
				...
				asITypeInfo *type = engine->GetTypeInfoById(typeId);
				asIScriptFunction *func = type ? type->GetMethodByName(("get_" + decl->declaration).c_str()) : 0;
				if( func )
				{
					it->second.funcMetadataMap.insert(map<int, vector<string> >::value_type(func->GetId(), decl->metadata));
					metadataBound = true;
				}
				func = type ? type->GetMethodByName(("set_" + decl->declaration).c_str()) : 0;
				if( func )
				{
					it->second.funcMetadataMap.insert(map<int, vector<string> >::value_type(func->GetId(), decl->metadata));
					metadataBound = true;
		...
		else if( decl->type == MDT_VAR )
		{
			...
			else
			{
				...
				for( asUINT i = 0; objectType && i < (asUINT)objectType->GetPropertyCount(); ++i )
				{
					const char *name;
		...
		if( !metadataBound )
			writeMetadataWarning(*decl, "target not found");
	}
#endif
...
int ScriptBuilder::SkipStatement(int pos)
{
	asUINT len = 0;
...
int ScriptBuilder::ExcludeCode(int pos)
{
	asUINT len = 0;
	int nested = 0;
...
void ScriptBuilder::OverwriteCode(int start, int len)
{
...
#if AS_PROCESS_METADATA == 1
int ScriptBuilder::ExtractMetadata(int pos, vector<string> &metadata)
{
	metadata.clear();
...
int ScriptBuilder::ExtractDeclaration(int pos, string &name, string &declaration, int &type)
{
	declaration = "";
	type = 0;
...
vector<string> ScriptBuilder::GetMetadataForType(int typeId)
{
...
vector<string> ScriptBuilder::GetMetadataForFunc(asIScriptFunction *func)
{
	if( func )
	{
...
vector<string> ScriptBuilder::GetMetadataForVar(int varIdx)
{
...
vector<string> ScriptBuilder::GetMetadataForTypeProperty(int typeId, int varIdx)
{
...
vector<string> ScriptBuilder::GetMetadataForTypeMethod(int typeId, asIScriptFunction *method)
{
	if( method )
	{
		map<int, SClassMetadata>::iterator typeIt = classMetadataMap.find(typeId);

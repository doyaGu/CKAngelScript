...
static void RegisterCKMeshMembers(asIScriptEngine *engine, const char *name) {
    ...
    r = engine->RegisterObjectMethod(name, "void RemoveAllCallbacks()", asFUNCTIONPR([](T *self) { self->RemoveAllCallbacks(); auto *man = ScriptManager::GetManager(self->GetCKContext()); if (man) { man->ReleaseCKObjectCallbacks(self->GetID()); man->ReleaseCKObjectData(self->GetID()); } }, (T *), void), asCALL_CDECL_OBJFIRST); assert(r >= 0);
...
void RegisterCKDataArray(asIScriptEngine *engine) {
    ...
    r = engine->RegisterObjectMethod("CKDataArray", "int GetStringValue(CKDWORD key, int c, string &out value)", asFUNCTIONPR([](CKDataArray *array, CKDWORD key, int c, std::string &value) -> int { char buffer[4096] = {}; int result = array->GetStringValue(key, c, buffer); if (result) value = buffer; else value.clear(); return result; }, (CKDataArray *, CKDWORD, int, std::string &), int), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKDataArray", "int GetElementStringValue(int i, int c, string &out value)", asFUNCTIONPR([](CKDataArray *array, int i, int c, std::string &value) -> int { char buffer[4096] = {}; int result = array->GetElementStringValue(i, c, buffer); if (result) value = buffer; else value.clear(); return result; }, (CKDataArray *, int, int, std::string &), int), asCALL_CDECL_OBJFIRST); assert(r >= 0);

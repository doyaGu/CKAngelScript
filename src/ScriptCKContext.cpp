...
void RegisterCKContext(asIScriptEngine *engine) {
    ...
    // Objects Management
    r = engine->RegisterObjectMethod("CKContext", "CKObject@ CreateObject(CK_CLASSID, const string &in = \"\", CK_OBJECTCREATION_OPTIONS = CK_OBJECTCREATION_NONAMECHECK)", asFUNCTIONPR([](CKContext *ctx, CK_CLASSID cid, CKSTRING name, CK_OBJECTCREATION_OPTIONS options) { return ctx->CreateObject(cid, name, options); }, (CKContext *, CK_CLASSID, CKSTRING, CK_OBJECTCREATION_OPTIONS), CKObject*), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "CKObject@ CreateObject(CK_CLASSID, const string &in, CK_OBJECTCREATION_OPTIONS, CK_LOADMODE &out)", asMETHODPR(CKContext, CreateObject, (CK_CLASSID, CKSTRING, CK_OBJECTCREATION_OPTIONS, CK_CREATIONMODE*), CKObject*), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "CKObject@ CopyObject(CKObject@, CKDependencies &in, const string &in = \"\", CK_OBJECTCREATION_OPTIONS = CK_OBJECTCREATION_NONAMECHECK)", asMETHODPR(CKContext, CopyObject, (CKObject*, CKDependencies*, CKSTRING, CK_OBJECTCREATION_OPTIONS), CKObject*), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "const XObjectArray &CopyObjects(const XObjectArray &in, CKDependencies &in, CK_OBJECTCREATION_OPTIONS = CK_OBJECTCREATION_NONAMECHECK, const string &in = \"\")", asMETHODPR(CKContext, CopyObjects, (const XObjectArray&, CKDependencies*, CK_OBJECTCREATION_OPTIONS, CKSTRING), const XObjectArray&), asCALL_THISCALL); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "CKERROR DestroyObject(CKObject@, CKDWORD, CKDependencies &in)", asMETHODPR(CKContext, DestroyObject, (CKObject*, CKDWORD, CKDependencies*), CKERROR), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "CKERROR DestroyObject(CK_ID, CKDWORD, CKDependencies &in)", asMETHODPR(CKContext, DestroyObject, (CK_ID, CKDWORD, CKDependencies*), CKERROR), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "CKERROR DestroyObjects(const XObjectArray &in, CKDWORD, CKDependencies &in)", asFUNCTIONPR([](CKContext *ctx, const XObjectArray &objects, CKDWORD flags, CKDependencies *deps) { return ctx->DestroyObjects(objects.Begin(), objects.Size(), flags, deps); }, (CKContext *, const XObjectArray &, CKDWORD, CKDependencies *), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    // Save/Load functions
    r = engine->RegisterObjectMethod("CKContext", "CKERROR Load(const string &in, CKObjectArray@, CK_LOAD_FLAGS, CKGUID &in)", asMETHODPR(CKContext, Load, (CKSTRING, CKObjectArray*, CK_LOAD_FLAGS, CKGUID*), CKERROR), asCALL_THISCALL); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "CKERROR Save(const string &in, CKObjectArray@, CKDWORD, CKDependencies &in, CKGUID &in)", asMETHODPR(CKContext, Save, (CKSTRING, CKObjectArray*, CKDWORD, CKDependencies*, CKGUID*), CKERROR), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "CKERROR LoadAnimationOnCharacter(const string &in, CKObjectArray@, CKCharacter@, CKGUID &in, bool = false)", asMETHODPR(CKContext, LoadAnimationOnCharacter, (CKSTRING, CKObjectArray*, CKCharacter*, CKGUID*, CKBOOL), CKERROR), asCALL_THISCALL); assert(r >= 0);

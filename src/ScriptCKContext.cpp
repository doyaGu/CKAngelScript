...
void RegisterCKContext(asIScriptEngine *engine) {
...
#endif
    r = engine->RegisterObjectMethod("CKContext", "void ChangeObjectDynamic(CKObject@ obj, bool setDynamic = true)", asFUNCTIONPR([](CKContext *ctx, CKObject *obj, bool setDynamic) { ctx->ChangeObjectDynamic(obj, setDynamic); }, (CKContext *, CKObject *, bool), void), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "const XObjectPointerArray &GetObjectListByType(CK_CLASSID cid, bool derived)", asFUNCTIONPR([](CKContext *ctx, CK_CLASSID cid, bool derived) -> const XObjectPointerArray & { return ctx->GetObjectListByType(cid, derived); }, (CKContext *, CK_CLASSID, bool), const XObjectPointerArray &), asCALL_CDECL_OBJFIRST); assert(r >= 0);
...
#else
    r = engine->RegisterObjectMethod("CKContext", "CKERROR Reset(bool toLevelScene = false)", asFUNCTIONPR([](CKContext *ctx, bool toLevelScene) { return ctx->Reset(toLevelScene); }, (CKContext *, bool), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
#endif
    r = engine->RegisterObjectMethod("CKContext", "bool IsPlaying()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsPlaying(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "bool IsReseted()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsReseted(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "bool IsInClearAll()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsInClearAll(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    // IHM
    // r = engine->RegisterObjectMethod("CKContext", "void SetInterfaceMode(bool mode = true, CKUICALLBACKFCT = null, NativePointer = null)", asMETHODPR(CKContext, SetInterfaceMode, (CKBOOL, CKUICALLBACKFCT, void*), void), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "bool IsInInterfaceMode()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsInInterfaceMode(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);

    r = engine->RegisterObjectMethod("CKContext", "CKERROR OutputToConsole(const string &in str, bool beep = true)", asFUNCTIONPR([](CKContext *ctx, const std::string &str, bool beep) { return ctx->OutputToConsole(const_cast<CKSTRING>(str.c_str()), beep); }, (CKContext *, const std::string &, bool), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "CKERROR OutputToInfo(const string &in format)", asFUNCTIONPR([](CKContext *ctx, const std::string &info) { return ctx->OutputToInfo(const_cast<CKSTRING>(info.c_str())); }, (CKContext *, const std::string &), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "CKERROR Select(const XObjectArray &in objects, bool clearSelection = true)", asFUNCTIONPR([](CKContext *ctx, const XObjectArray &objects, bool clearSelection) { return ctx->Select(objects, clearSelection); }, (CKContext *, const XObjectArray &, bool), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "CKERROR UICopyObjects(const XObjectArray &in, bool iClearClipboard = true)", asFUNCTIONPR([](CKContext *ctx, const XObjectArray &objects, bool iClearClipboard) { return ctx->UICopyObjects(objects, iClearClipboard); }, (CKContext *, const XObjectArray &, bool), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "CKBaseManager@ GetManagerByName(const string &in name)", asFUNCTIONPR([](CKContext *ctx, const std::string &name) { return ctx->GetManagerByName(const_cast<CKSTRING>(name.c_str())); }, (CKContext *, const std::string &), CKBaseManager*), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "bool IsManagerActive(CKBaseManager@ manager)", asFUNCTIONPR([](CKContext *ctx, CKBaseManager *manager) -> bool { return ctx->IsManagerActive(manager); }, (CKContext *, CKBaseManager *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "bool HasManagerDuplicates(CKBaseManager@ manager)", asFUNCTIONPR([](CKContext *ctx, CKBaseManager *manager) -> bool { return ctx->HasManagerDuplicates(manager); }, (CKContext *, CKBaseManager *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "void ActivateManager(CKBaseManager@ manager, bool active)", asFUNCTIONPR([](CKContext *ctx, CKBaseManager *manager, bool active) { ctx->ActivateManager(manager, active); }, (CKContext *, CKBaseManager *, bool), void), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    // Profiling Functions
    r = engine->RegisterObjectMethod("CKContext", "void EnableProfiling(bool enable)", asFUNCTIONPR([](CKContext *ctx, bool enable) { ctx->EnableProfiling(enable); }, (CKContext *, bool), void), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "bool IsProfilingEnable()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsProfilingEnable(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "CKERROR Save(const string &in fileName, CKObjectArray@ list, CKDWORD saveFlags, CKDependencies &in dependencies = void, CKGUID &in readerGuid = void)", asFUNCTIONPR([](CKContext *ctx, const std::string &fileName, CKObjectArray *list, CKDWORD saveFlags, CKDependencies *dependencies, CKGUID *readerGuid) { return ctx->Save(const_cast<CKSTRING>(fileName.c_str()), list, saveFlags, dependencies, readerGuid); }, (CKContext *, const std::string &, CKObjectArray *, CKDWORD, CKDependencies *, CKGUID *), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "CKERROR LoadAnimationOnCharacter(const string &in fileName, CKObjectArray@ list, CKCharacter@ carac, CKGUID &in readerGuid = void, bool asDynamicObjects = false)", asFUNCTIONPR([](CKContext *ctx, const std::string &fileName, CKObjectArray *list, CKCharacter *carac, CKGUID *readerGuid, bool asDynamicObjects) { return ctx->LoadAnimationOnCharacter(const_cast<CKSTRING>(fileName.c_str()), list, carac, readerGuid, asDynamicObjects); }, (CKContext *, const std::string &, CKObjectArray *, CKCharacter *, CKGUID *, bool), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    // r = engine->RegisterObjectMethod("CKContext", "CK_LOADMODE LoadVerifyObjectUnicity(const string &in oldName, CK_CLASSID cid, const string &in newName, CKObject@ &out newObj)", asFUNCTIONPR([](CKContext *ctx, const std::string &oldName, CK_CLASSID cid, const std::string &newName, CKObject **newObj) { return ctx->LoadVerifyObjectUnicity(const_cast<CKSTRING>(oldName.c_str()), cid, const_cast<CKSTRING>(newName.c_str()), newObj); }, (CKContext *, const std::string &, CK_CLASSID, const std::string &, CKObject **), CK_LOADMODE), asCALL_CDECL_OBJFIRST); assert(r >= 0);

    r = engine->RegisterObjectMethod("CKContext", "bool IsInLoad()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsInLoad(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "bool IsInSave()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsInSave(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKContext", "bool IsRunTime()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsRunTime(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "bool ProcessDebugStep()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->ProcessDebugStep(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKContext", "bool IsInDynamicCreationMode()", asFUNCTIONPR([](CKContext *ctx) -> bool { return ctx->IsInDynamicCreationMode(); }, (CKContext *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
}

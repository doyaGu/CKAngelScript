...
bool RunScriptInfoExportSelfTest(std::string &error) {
    const std::string escaped = JsonEscape("quote\" slash\\ newline\n tab\t control\x01");
    if (escaped != "quote\\\" slash\\\\ newline\\n tab\\t control\\u0001") {
        error = "ScriptInfo export self-test failed JSON string escaping.";
        return false;
    }

    const std::string object = WriteObject({
        {"name", "Scene"},
        {"schemaVersion", "1", true},
        {"enabled", JsonBool(true), true}
    }, 0);
    ...
    if (object != expectedObject) {
        error = "ScriptInfo export self-test failed deterministic object output.";
        return false;
    }

    std::vector<std::string> items = {
        WriteObject({{"namespace", "Runtime"}, {"name", "Tick"}}, 2),
        WriteObject({{"namespace", "BB"}, {"name", "Input"}}, 2),
        WriteObject({{"namespace", "Scene"}, {"name", "FindObject"}}, 2)
    };
    std::sort(items.begin(), items.end());
    const std::string array = WriteArray(items, 0);
    if (array.find("\"namespace\": \"BB\"") == std::string::npos ||
        array.find("\"namespace\": \"BB\"") > array.find("\"namespace\": \"Runtime\"") ||
        array.find("\"namespace\": \"Runtime\"") > array.find("\"namespace\": \"Scene\"")) {
        error = "ScriptInfo export self-test failed stable sorted output.";
        return false;
    }

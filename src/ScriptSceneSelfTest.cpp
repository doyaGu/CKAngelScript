...
bool RunScriptSceneSelfTest(CKContext *context, asIScriptEngine *engine, std::string &error) {
    ...
    const char *source =
        "CKObject@ __ckas_entity3d_object(Entity3DRef@ ref) {\n"
        "  ObjectRef@ object = ref;\n"
        "  return object !is null ? object.Object() : null;\n"
        "}\n"
        "CK3dEntity@ __ckas_entity3d_raw(Entity3DRef@ ref) {\n"
        "  return cast<CK3dEntity>(__ckas_entity3d_object(ref));\n"
        "}\n"
        ...
        "}\n"
        "CKTexture@ __ckas_texture_raw(TextureRef@ ref) {\n"
        "  ObjectRef@ object = ref;\n"
        "  return object !is null ? cast<CKTexture>(object.Object()) : null;\n"
        "}\n"
        "CKMesh@ __ckas_mesh_raw(MeshRef@ ref) {\n"
        "  ObjectRef@ object = ref;\n"
        "  return object !is null ? cast<CKMesh>(object.Object()) : null;\n"
        "}\n"
        ...
        "  CK3dEntity@ createdRaw = __ckas_entity3d_raw(created);\n"
        "  if (created is null || !created.valid || createdRaw is null) return 10;\n"
        "  VxIntersectionDesc rayDesc;\n"
        "  createdRaw.RayIntersection(VxVector(0.0f, 0.0f, -1.0f), VxVector(0.0f, 0.0f, 1.0f), rayDesc, null);\n"
        "  VxIntersectionDesc copiedRayDesc(rayDesc);\n"
        "  VxIntersectionDesc assignedRayDesc;\n"
        "  assignedRayDesc = copiedRayDesc;\n"
        "  if (!created.IsDynamic() || created.Name() != objectName || created.ClassId() != CKCID_3DENTITY) return 11;\n"
        "  CKObject@ createdObject = __ckas_entity3d_object(created);\n"
        ...
        "  }\n"
        "  ObjectRef@ rawRef = Scene::Ref(ctx, __ckas_entity3d_object(created));\n"
        ...
        "  if (typedFound is null || __ckas_entity3d_raw(typedFound) is null || typedFound.Id() != createdId) return 15;\n"
        ...
        "  __ckas_entity3d_object(dupA).SetName(duplicateName);\n"
        "  __ckas_entity3d_object(dupB).SetName(duplicateName);\n"
        "  ctx.Context.ChangeObjectDynamic(__ckas_entity3d_object(dupA), true);\n"
        "  ctx.Context.ChangeObjectDynamic(__ckas_entity3d_object(dupB), true);\n"
        ...
        "    CKSprite3D@ scopedSpriteRaw = cast<CKSprite3D>(__ckas_entity3d_object(scopedSprite));\n"
        "    CKMaterial@ scopedMaterialRaw = __ckas_material_raw(scopedMaterial);\n"
        "    CKTexture@ scopedTextureRaw = __ckas_texture_raw(scopedTexture);\n"
        ...
        "    scopedSpriteRaw.SetMaterial(__ckas_material_raw(scopedMaterial));\n"
        "    CK2dEntity@ scoped2DRaw = __ckas_entity2d_raw(scoped2D);\n"
        ...
        "    scoped2DRaw.SetMaterial(__ckas_material_raw(scoped2DMaterial));\n"
        "    if (!Scene::IsInScene(ctx, scene, scoped2DMaterial)) return 96;\n"
        "    MeshRef@ scopedMesh = Scene::CreateMesh(ctx, scopedMeshName, true);\n"
        "    if (scopedMesh is null || !scopedMesh.valid || __ckas_mesh_raw(scopedMesh) is null) return 97;\n"
        "    CK3dEntity@ dupARaw = __ckas_entity3d_raw(dupA);\n"
        "    if (dupARaw is null || dupARaw.AddMesh(__ckas_mesh_raw(scopedMesh)) != CK_OK || dupARaw.GetMeshCount() <= 0 || dupARaw.GetMesh(0) is null) return 99;\n"
        "    dupARaw.SetCurrentMesh(__ckas_mesh_raw(scopedMesh), false); CKMesh@ maybeCurrentMesh = dupARaw.GetCurrentMesh();\n"
        ...
        "    CKBehavior@ scopedBehaviorRaw = scopedBehaviorObject !is null ? cast<CKBehavior>(scopedBehaviorObject.Object()) : null;\n"
        "    CKBeObject@ behaviorOwner = cast<CKBeObject>(__ckas_entity3d_object(dupA));\n"
        ...
        "  CK3dEntity@ parent3DRaw = __ckas_entity3d_raw(parent3D);\n"
        "  CK3dEntity@ child3DRaw = __ckas_entity3d_raw(child3D);\n"
        ...
        "  CK2dEntity@ parent2DRaw = __ckas_entity2d_raw(parent2D);\n"
        "  CK2dEntity@ child2DRaw = __ckas_entity2d_raw(child2D);\n"
        ...
        "  ctx.Context.ChangeObjectDynamic(__ckas_entity3d_object(persistent), false);\n"
        "  if (persistent.IsDynamic()) return 22;\n"
        "  if (Scene::Destroy(ctx, persistent)) return 23;\n"
        "  if (!Scene::Destroy(ctx, persistent, true)) {\n"
        "    ctx.Context.ChangeObjectDynamic(__ckas_entity3d_object(persistent), true);\n"
        "    if (!Scene::Destroy(ctx, persistent, true)) return 24;\n"
        "  }\n"
        "  if (!Scene::Destroy(ctx, created)) return 25;\n"
        "  if (created.IsValid() || __ckas_entity3d_object(created) !is null || __ckas_entity3d_raw(created) !is null) return 26;\n"
        ...
        "  if (__ckas_entity3d_raw(child3D) !is null) return 58;\n"
        "  if (__ckas_entity2d_raw(child2D) !is null) return 60;\n"

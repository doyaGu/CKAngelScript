...
static bool RunBehaviorBridgeScriptSelfTest(CKContext *context,
    ...
    source += "    ParamInfo@ pinInfo = layout.Pin(pinIndex);\n";
    source += "    if (pinInfo is null || pinInfo.name == \"\" || pinInfo.TypeName() == \"\") return 994;\n";
    source += "    pinInfo.kind; pinInfo.index; pinInfo.TypeGuid(); pinInfo.DataSize(); pinInfo.Describe();\n";
    source += "    int poutIndex = layout.FindPout(\"pOut 0\");\n";
    source += "    if (poutIndex >= 0) { ParamInfo@ poutInfo = layout.Pout(poutIndex); if (poutInfo is null || poutInfo.name == \"\") return 995; }\n";
    source += "    int settingIndex = layout.FindSetting(\"Settings\");\n";
    source += "    if (settingIndex >= 0) { ParamInfo@ settingInfo = layout.Setting(settingIndex); if (settingInfo is null || settingInfo.name == \"\") return 996; }\n";
    source += "    int localIndex = layout.FindLocal(\"State\");\n";
    source += "    if (localIndex >= 0) { ParamInfo@ localInfo = layout.Local(localIndex); if (localInfo is null || localInfo.name == \"\") return 997; }\n";

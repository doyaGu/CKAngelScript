...
static bool RunBehaviorBridgeScriptSelfTest(CKContext *context,
    ...
    source += "    if (e3d !is null) { VxVector p; VxQuaternion q; VxBbox box; e3d.SetPosition(p); e3d.SetPosition(0.0f, 0.0f, 0.0f); e3d.GetPosition(p); e3d.Translate(p); e3d.Translate(0.0f, 0.0f, 0.0f); e3d.SetQuaternion(q); e3d.GetQuaternion(q); e3d.SetScale(p); e3d.SetScale(1.0f, 1.0f, 1.0f); e3d.GetScale(p); e3d.LookAt(p); Entity3DRef@ parent3d = e3d.Parent(); Entity3DRef@ child3d = e3d.Child(0); array<Entity3DRef@>@ children3d = e3d.Children(); e3d.ChildCount(); e3d.SetParent(parent3d); e3d.AddChild(child3d); e3d.RemoveChild(child3d); e3d.IsAncestorOf(child3d); e3d.IsDescendantOf(parent3d); e3d.SetPickable(); e3d.IsPickable(); MeshRef@ currentMesh = e3d.CurrentMesh(); e3d.SetCurrentMesh(oneMesh); e3d.MeshCount(); e3d.Mesh(0); e3d.AddMesh(oneMesh); e3d.SetOrientation(p, p, p); e3d.GetOrientation(p, p, p); e3d.SetDirection(p); e3d.GetDirection(p); e3d.SetUp(p); e3d.GetUp(p); e3d.SetRight(p); e3d.GetRight(p); e3d.GetBoundingBox(box); e3d.SetBoundingBox(box); e3d.Radius(); e3d.Material(); }\n";
    source += "    if (e2d !is null) { Vx2DVector p2; VxRect r2; VxRect src2; e2d.SetPosition(p2); e2d.SetPosition(0.0f, 0.0f); e2d.GetPosition(p2); e2d.SetSize(p2); e2d.SetSize(1.0f, 1.0f); e2d.GetSize(p2); e2d.SetRect(r2); e2d.GetRect(r2); e2d.SetSourceRect(r2); e2d.GetSourceRect(r2); e2d.UseSourceRect(); e2d.IsUsingSourceRect(); e2d.SetMaterial(oneMaterial); MaterialRef@ e2dMat = e2d.Material(); e2d.SetPickable(); e2d.IsPickable(); e2d.SetClipToParent(); e2d.IsClipToParent(); e2d.SetBackground(); e2d.IsBackground(); e2d.EnableRatioOffset(); e2d.IsRatioOffset(); e2d.EnableClipToCamera(); e2d.IsClippedToCamera(); e2d.Flags(); e2d.SetFlags(0); e2d.ModifyFlags(0); e2d.SetExtents(src2, r2); e2d.GetExtents(src2, r2); Entity2DRef@ parent2d = e2d.Parent(); Entity2DRef@ child2d = e2d.Child(0); array<Entity2DRef@>@ children2d = e2d.Children(); e2d.ChildCount(); e2d.SetParent(parent2d); e2d.IsAncestorOf(child2d); e2d.IsDescendantOf(parent2d); }\n";
    ...
    source += "    if (material !is null) { VxColor color; material.Texture(); material.SetTexture(texture); material.Ambient(); material.SetAmbient(color); material.Diffuse(); material.SetDiffuse(color); material.Specular(); material.SetSpecular(color); material.Emissive(); material.SetEmissive(color); material.Power(); material.SetPower(1.0f); material.TextureBlendMode(); material.SetTextureBlendMode(VXTEXTUREBLEND_MODULATE); material.SourceBlend(); material.SetSourceBlend(VXBLEND_SRCALPHA); material.DestBlend(); material.SetDestBlend(VXBLEND_INVSRCALPHA); material.AlphaBlendEnabled(); material.EnableAlphaBlend(); material.AlphaFunc(); material.SetAlphaFunc(VXCMP_ALWAYS); material.AlphaRef(); material.SetAlphaRef(0); }\n";
    source += "    if (found !is null) { found.IsValid(); bool valid = found.valid; found.Error(); found.Describe(); found.Id(); found.Name(); found.ClassId(); found.IsDynamic(); found.Object(); found.SetName(\"__missing__\"); found.SetDynamic(); found.Show(); found.IsVisible(); found.ObjectFlags(); found.ModifyObjectFlags(0); BehaviorRef@ castBehavior = cast<BehaviorRef>(found); ParamRef@ castParam = cast<ParamRef>(found); SceneObjectRef@ castSceneObject = cast<SceneObjectRef>(found); }\n";
    source += "    SceneObjectRef@ sceneAsObject = currentScene;\n";
    source += "    if (sceneAsObject !is null) { sceneAsObject.IsInCurrentScene(); sceneAsObject.IsInScene(currentScene); }\n";

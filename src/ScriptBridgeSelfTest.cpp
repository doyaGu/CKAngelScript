...
static bool RunBehaviorBridgeScriptSelfTest(CKContext *context,
    ...
    source += "    BBSpec@ spec = BB::Require(ctx, \"__missing__\"); BBSlot@ input = spec !is null ? spec.In(\"In\") : null; BBSlot@ output = spec !is null ? spec.Out(\"Out\") : null; BBSlot@ pinSlot = spec !is null ? spec.Pin(\"Value\") : null; BBSlot@ poutSlot = spec !is null ? spec.Pout(\"Value\") : null;\n";

...
void RegisterCKSoundManager(asIScriptEngine *engine) {
    ...
    r = engine->RegisterObjectMethod("CKSoundManager", "void Play(CKWaveSound@ sound, NativePointer source, bool loop)", asFUNCTIONPR([](CKSoundManager *self, CKWaveSound *sound, void *source, bool loop) { self->Play(sound, source, loop); }, (CKSoundManager *, CKWaveSound *, void *, bool), void), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKSoundManager", "bool IsPlaying(NativePointer source)", asFUNCTIONPR([](CKSoundManager *self, void *source) -> bool { return self->IsPlaying(source); }, (CKSoundManager *, void *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKSoundManager", "void Update3DSettings(NativePointer source, CK_SOUNDMANAGER_CAPS settingsoptions, CKWaveSound3DSettings &out settings, bool set = true)", asFUNCTIONPR([](CKSoundManager *self, void *source, CK_SOUNDMANAGER_CAPS settingsoptions, CKWaveSound3DSettings &settings, bool set) { self->Update3DSettings(source, settingsoptions, settings, set); }, (CKSoundManager *, void *, CK_SOUNDMANAGER_CAPS, CKWaveSound3DSettings &, bool), void), asCALL_CDECL_OBJFIRST); assert(r >= 0);
...
void RegisterCKMidiManager(asIScriptEngine *engine) {
    ...
    r = engine->RegisterObjectMethod("CKMidiManager", "CKERROR SetSoundFileName(NativePointer source, const string &in filename)", asFUNCTIONPR([](CKMidiManager *self, void *source, const std::string &filename) { return self->SetSoundFileName(source, const_cast<CKSTRING>(filename.c_str())); }, (CKMidiManager *, void *, const std::string &), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKMidiManager", "string GetSoundFileName(NativePointer source)", asFUNCTIONPR([](CKMidiManager *self, void *source) -> std::string { return self->GetSoundFileName(source); }, (CKMidiManager *, void *), std::string), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKMidiManager", "CKERROR Pause(NativePointer source, bool pause = true)", asFUNCTIONPR([](CKMidiManager *self, void *source, bool pause) -> CKERROR { return self->Pause(source, pause); }, (CKMidiManager *, void *, bool), CKERROR), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKMidiManager", "bool IsPlaying(NativePointer source)", asFUNCTIONPR([](CKMidiManager *self, void *source) -> bool { return self->IsPlaying(source); }, (CKMidiManager *, void *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKMidiManager", "bool IsPaused(NativePointer source)", asFUNCTIONPR([](CKMidiManager *self, void *source) -> bool { return self->IsPaused(source); }, (CKMidiManager *, void *), bool), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("CKMidiManager", "CKERROR Time(NativePointer source, CKDWORD &out ticks)", asMETHODPR(CKMidiManager, Time, (void*, CKDWORD*), CKERROR), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKMidiManager", "CKDWORD MillisecsToTicks(NativePointer source, CKDWORD msOffset)", asMETHODPR(CKMidiManager, MillisecsToTicks, (void*, CKDWORD), CKDWORD), asCALL_THISCALL); assert(r >= 0);
    r = engine->RegisterObjectMethod("CKMidiManager", "CKDWORD TicksToMillisecs(NativePointer source, CKDWORD tkOffset)", asMETHODPR(CKMidiManager, TicksToMillisecs, (void*, CKDWORD), CKDWORD), asCALL_THISCALL); assert(r >= 0);
}

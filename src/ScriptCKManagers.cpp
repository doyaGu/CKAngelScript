...
void RegisterCKMidiManager(asIScriptEngine *engine) {
    ...
    r = engine->RegisterObjectMethod("CKMidiManager", "string GetSoundFileName(uintptr_t source)", asFUNCTIONPR([](CKMidiManager *self, void *source) -> std::string { return ScriptStringify(self->GetSoundFileName(source)); }, (CKMidiManager *, void *), std::string), asCALL_CDECL_OBJFIRST); assert(r >= 0);

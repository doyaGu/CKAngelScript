...
bool RunScriptRuntimeSelfTest(CKContext *context, asIScriptEngine *engine, std::string &error) {
    ...
    const char *source =
        ...
        "  if (material !is null) { CKMaterial@ rawMaterial = __ckas_material_raw(material); CKTexture@ rawTexture = texture !is null ? __ckas_texture_raw(texture) : null; if (rawMaterial !is null) { rawMaterial.GetTexture(); rawMaterial.SetTexture(rawTexture); } }\n"

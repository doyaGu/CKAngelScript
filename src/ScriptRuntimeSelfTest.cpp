...
bool RunScriptRuntimeSelfTest(CKContext *context, asIScriptEngine *engine, std::string &error) {
    ...
    const char *source =
        ...
        "  if (material !is null) { CKMaterial@ rawMaterial = material.Material(); CKTexture@ rawTexture = texture !is null ? texture.Texture() : null; if (rawMaterial !is null) { rawMaterial.GetTexture(); rawMaterial.SetTexture(rawTexture); } }\n"
